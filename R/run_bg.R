...
#' @param record_attempts Logical. When TRUE, attaches the number of
#'   attempts used as attr(value, "retry_attempts") on success (default
#'   FALSE, so atomic return values are never altered by default).
#'
...
retry <- function(
  ...
  # ---- Validate arguments (fail fast, clear message) ----------------------
  .retry_check_num  <- function(x, nm, min_val = 0) {
    if (!is.numeric(x) || length(x) != 1L || is.na(x) || !is.finite(x) ||
        x < min_val) {
      stop(paste0("retry(): '", nm, "' must be a single finite number >= ",
                  min_val, "."), call. = FALSE)
    }
  }
  if (!is.numeric(times) || length(times) != 1L || is.na(times) ||
      !is.finite(times) || times < 1) {
    stop("retry(): 'times' must be a single finite number >= 1.",
         call. = FALSE)
  }
  .retry_check_num(wait, "wait")
  .retry_check_num(max_wait, "max_wait")
  if (!is.logical(backoff) || length(backoff) != 1L || is.na(backoff)) {
    stop("retry(): 'backoff' must be TRUE or FALSE.", call. = FALSE)
  }
  if (!is.logical(jitter) || length(jitter) != 1L || is.na(jitter)) {
    stop("retry(): 'jitter' must be TRUE or FALSE.", call. = FALSE)
  }
  if (!is.logical(silent) || length(silent) != 1L || is.na(silent)) {
    stop("retry(): 'silent' must be TRUE or FALSE.", call. = FALSE)
  }
  if (!is.null(on) && !is.character(on)) {
    stop("retry(): 'on' must be NULL or a character vector.", call. = FALSE)
  }
  ...
  for (attempt in seq_len(times)) {
    ...
    if (!inherits(result, "orchrd_retry_error")) {
      ...
      if (isTRUE(record_attempts)) {
        attr(result, "retry_attempts") <- attempt
      }
      return(result)
    }
...
retry_labeled <- function(
  ...
  if (!is.character(label) || length(label) != 1L || is.na(label)) {
    stop("retry_labeled(): 'label' must be a single character string.",
         call. = FALSE)
  }
  expr_q    <- substitute(expr)
  finally_q <- substitute(finally)
